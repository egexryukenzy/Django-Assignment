...
def dashboard(request):
    ...
    if user.is_admin:
        ...
        projects = Project.objects.filter(
            Q(owner=user) | Q(id__in=member_project_ids)
        ).distinct().order_by('-created_at')

    my_cards = Card.objects.filter(assignees=user).exclude(status='done').order_by('deadline')[:5]
    overdue_cards = [c for c in Card.objects.filter(assignees=user).exclude(status='done') if c.is_overdue]
    notifications = user.notifications.filter(is_read=False)[:5]
    total_tasks = Card.objects.filter(assignees=user).count()
    done_tasks = Card.objects.filter(assignees=user, status='done').count()
    doing_tasks = Card.objects.filter(assignees=user, status='doing').count()
    todo_tasks = Card.objects.filter(assignees=user, status='todo').count()
...
def project_list(request):
    ...
    if user.is_admin:
        ...
        projects = Project.objects.filter(Q(owner=user) | Q(id__in=member_ids)).distinct().order_by('-created_at')
...
def board_create(request, project_id):
    ...
    board = Board.objects.create(project=project, name=name, position=project.boards.count())
    List.objects.create(board=board, title='To Do / ត្រូវធ្វើ', position=0)
    List.objects.create(board=board, title='Doing / កំពុងធ្វើ', position=1)
    List.objects.create(board=board, title='Done / បានធ្វើ', position=2)
...
def card_create(request, list_id):
    ...
    card = Card.objects.create(
        list=lst, created_by=request.user, title=title,
...
def card_detail(request, card_id):
    ...
    assigned_ids = CardAssignment.objects.filter(card=card).values_list('user_id', flat=True)
    ...
    if request.method == 'POST':
        ...
        if action == 'edit':
            ...
            status_to_title = {'todo': 'To Do / ត្រូវធ្វើ', 'doing': 'Doing / កំពុងធ្វើ', 'done': 'Done / បានធ្វើ'}
            ...
            if f:
                Attachment.objects.create(card=card, uploaded_by=request.user, file=f, file_name=f.name)
...
def search(request):
    ...
    context = {
        'q': q, 'cards': cards, 'projects': projects,
        'unread_count': request.user.notifications.filter(is_read=False).count(),
    }
...
def admin_reports(request):
    ...
    overdue = [c for c in Card.objects.exclude(status='done').filter(deadline__isnull=False) if c.is_overdue]
